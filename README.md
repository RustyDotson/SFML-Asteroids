...
After you have cmake setup, use the command `cmake -B build` (even if the folder `build` does not exist, cmake will automatically create this folder from the command. `cmake -B build` essentially lets cmake generate all the necessary build files in order to compile the program). Then, use the command `cmake --build build` in the root directory. The output program (depending on the OS) should be located somewhere like: build/bin/main.
